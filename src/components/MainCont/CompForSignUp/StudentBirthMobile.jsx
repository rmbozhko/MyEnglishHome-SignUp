export const StudentBirthMobile = ({ isValideDate, register, c, watch, errors, now }) => {
    return (
        <div className={c.center}>
            <span>Дата народження студента</span>
            ...
            {errors?.day ?
                <div className={`${c.error} + ' ' + ${c.nameError}`}>{errors?.day && <><i className="material-icons">&#xe002;</i>&nbsp;&nbsp;<span>{errors?.day?.message}</span></>}</div> : null}
            ...
            {errors?.year || !isValideDate ? <>
                <div className={`${c.error} + ' ' + ${c.nameError}`}>{!isValideDate && <><i className="material-icons">&#xe002;</i>&nbsp;&nbsp;<span>Такої дати не існує</span></>}</div>
