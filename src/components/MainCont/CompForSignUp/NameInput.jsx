...
export const NameInput = ({ isMobile, clearErrors, watch, setError, register, title, firstName, secondName, errors, req, c, }) => {
    return (
        <>
            {!isMobile && <span>{title}<span className={c.neces}>&#9913;</span></span>}
            ...
            {errors?.[firstName] || errors?.[secondName] ? <div className={c.center}>
                {!isMobile ? <>
                    ...
                    : <>
                        <div className={`${c.error} + ' ' + ${c.nameError}`}>{(errors?.[firstName] || errors?.[secondName]) && <><i className="material-icons">&#xe002;</i>&nbsp;&nbsp;<span>{errors?.[firstName]?.message || errors?.[secondName]?.message}</span></>}</div>
                    </>}</div> : null}
        </>
    )
}
